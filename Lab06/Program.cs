        ...
        static void Main()
        {
            ...
            Console.WriteLine("\n== STAGE 4 ==\n");
            {
                for (int i = 0; i < objects.Length; i++)
                {
                    ...
                    if (!(objects[i] is Shape)) Console.WriteLine($"Object is not a shape");
                    else 
                    {
                        Console.WriteLine($"Object is a shape");
                        Console.WriteLine($"Shape has a circuit {((Shape)objects[i]).Circuit()}");
                        
                        if (!(objects[i] is Polygon)) Console.WriteLine($"Object is not a polygon");
                        else
                        {
                            Console.WriteLine($"Object is a polygon");
                            Console.WriteLine($"List of points in polygon:");
                            for (int j = 0; j < ((Polygon)objects[i]).Points.Length; j++)
                            {
                                Console.WriteLine((((Polygon)objects[i]).Points)[j]);
                            }
                        }

                        if (objects[i] is Circle)
                        {
                            Console.WriteLine($"Shape is a circle that has center at {((Circle)(objects[i])).P} with radius of {((Circle)(objects[i])).R}");                            
                        }

                        if (objects[i] is Rectangle)
                        {
                            Console.WriteLine($"Shape is a rectangle with diagonal of {((Rectangle)(objects[i])).diag}");
                        }
                    }
